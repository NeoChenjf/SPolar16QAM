...
r=mod(K,7);
code=zeros(N,1);
origin_data=randsrc((4*B+r),1,[0 1;0.5 0.5]);
data_encode=[];
data_t=origin_data.';
for i_crc=1:B
    crc_code= crc_encode(G,data_t(4*(i_crc-1)+1:4*i_crc));
    data_encode=[data_encode crc_code];
end
if r~=0
    data_encode=[data_encode data_t(end-r+1:end)];
end
code(I_set)=data_encode;
...
%     polar_info_sc=polar_info_sc(S_size+1,end);
    data_t=polar_info_sc';
    data_check=[];
    for i_check=1:B
        crc_check_result= crc_check(G , data_t(7*(i_check-1)+1:7*i_check));
        data_check=[data_check crc_check_result];
    end
    if r~=0
        data_check=[data_check data_t(end-r+1:end)];
    end
%     for i_crc=1:B
%     crc_code= crc_encode(G,origin_data(4*(i_crc-1)+1:4*i_crc));
%     data_encode=[data_encode crc_code];
% end
% if r~=0
%     data_encode=[data_encode origin_data(end-r+1:end)];
% end
% code(I_set)=data_encode;
% code(S_set)=shaped_bits;
data_check=data_check';

   if any(data_check~=origin_data)
        blenum_sc=blenum_sc+1;
   end
   
   
   nfails=sum(data_check~=origin_data);
    BER(i)=BER(i)+nfails;
